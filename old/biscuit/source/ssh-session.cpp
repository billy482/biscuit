...
Ssh::SshSession::SshSession(Ssh& ssh) : m_ssh(ssh) {}
...
bool Ssh::SshSession::do_connection() {
	...
	QByteArray host = this->m_ssh.m_host.hostname().toUtf8();

	bool connected = false;
	if (inet_pton(AF_INET6, host, &ip_v6.sin6_addr) == 1) {
		ip_v6.sin6_family = AF_INET6;
		ip_v6.sin6_port = htons(this->m_ssh.m_port);

		this->m_ssh.m_logger->info("Connecting to {}:{}", static_cast<const char *>(host), this->m_ssh.m_port);

		this->m_socket = socket(AF_INET6, SOCK_STREAM, 0);
		if (this->m_socket < 0)
			this->m_ssh.m_logger->error("Error while creating socket because {}", strerror(errno));
		else if (connect(this->m_socket, reinterpret_cast<const struct sockaddr *>(&ip_v6), sizeof(ip_v6)) != 0)
			this->m_ssh.m_logger->error("Error while connecting to {}:{} because {}", static_cast<const char *>(host), this->m_ssh.m_port, strerror(errno));
		else {
			connected = true;
			this->m_ssh.m_logger->info("Connected to {}:{}", static_cast<const char *>(host), this->m_ssh.m_port);
		}
	} else if (inet_pton(AF_INET, host, &ip_v4.sin_addr) == 1) {
		ip_v4.sin_family = AF_INET;
		ip_v4.sin_port = htons(this->m_ssh.m_port);
		...
		this->m_ssh.m_logger->info("Resolving {]", static_cast<const char *>(host));
		int error = getaddrinfo(host, nullptr, &hints, &res);
		if (error != 0)
			this->m_ssh.m_logger->error("Error while resolving {} because {}", static_cast<const char *>(host), gai_strerror(error));
		else {
			for (struct addrinfo * ptr = res; ptr != nullptr and not connected; ptr = ptr->ai_next) {
				...
				if (inet_ntop(ptr->ai_family, ptr->ai_addr, buffer, 40) != nullptr)
					this->m_ssh.m_logger->info("Connecting to {}:{}", buffer, this->m_ssh.m_port);
				else
					this->m_ssh.m_logger->info("Connecting to {}:{}", static_cast<const char *>(host), this->m_ssh.m_port);
...

	{
		...
		this->m_ssh.m_logger->debug("finger print (base64): {}", static_cast<const char *>(finger_print.toBase64()));
		this->m_ssh.m_logger->debug("finger print (hex): {}", static_cast<const char *>(finger_print.toHex()));
	}

	QByteArray user_name = this->m_ssh.m_user.toUtf8();
	if (this->m_ssh.m_use_ssh_agent) {
		LIBSSH2_AGENT * agent = libssh2_agent_init(this->m_session);
		int rc = libssh2_agent_connect(agent);
		if (rc == 0) {
			this->m_ssh.m_logger->info("connected to agent");
			...
			if (rc != 0)
				...
				for (;;) {
					...
					if (rc == 0) {
						authenticated = true;
						this->m_ssh.m_logger->info("connected");
						break;
					}
	...
	if (not this->m_ssh.m_identity_file.isNull() and strstr(user_auth_list, "publickey") != nullptr) {
		const QByteArray identity_file = this->m_ssh.m_identity_file.toUtf8();
		const QByteArray identity_file_pub = (this->m_ssh.m_identity_file + ".pub").toUtf8();
		const QString prompt = QString("%1@%2").arg(this->m_ssh.m_user, this->m_ssh.m_identity_file);
		QString password;
...
void Ssh::SshSession::do_disconnection() {
	if (this->m_session != nullptr) {
...
void Ssh::SshSession::log_error() {
	char * error_message = nullptr;
	int error_len = 0;
	libssh2_session_last_error(this->m_session, &error_message, &error_len, 1);
	this->m_ssh.m_logger->error(error_message);
}

void Ssh::SshSession::log_error(int type, const char * message) {
	...
	if (rc > 0) {
		...
		this->m_ssh.m_logger->debug("supported {}: {}", message, buffer.str());
	} else
		this->m_ssh.m_logger->error("Error while listing {}", message);
}

QIODevice * Ssh::SshSession::open(const FileInfo& file) {
	QByteArray path = file.path().toUtf8();
	LIBSSH2_SFTP_HANDLE * handle = libssh2_sftp_open(this->m_channel, path, LIBSSH2_FXF_READ, 0);
	if (handle != nullptr)
...
bool Ssh::SshSession::scan_directory(const QString& path) {
	const QByteArray byte_path = path.toUtf8();
	LIBSSH2_SFTP_HANDLE * sftp_handle = libssh2_sftp_opendir(this->m_channel, byte_path);
	...
	if (files.size() > 0) {
		...
		this->m_ssh.m_paths.push(files);
	}
	
	return true;
}

bool Ssh::SshSession::start_ftp_session() {
	if (this->m_session == nullptr)
		return false;
	...
	this->m_channel = libssh2_sftp_init(this->m_session);
	return this->m_channel != nullptr;
}
