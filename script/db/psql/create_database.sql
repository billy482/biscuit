...
CREATE TABLE files (
	...
	data BYTEA NOT NULL,
	key INTEGER NOT NULL REFERENCES keys(id) ON UPDATE CASCADE ON DELETE RESTRICT,
	device INTEGER NOT NULL REFERENCES devices(id) ON UPDATE CASCADE ON DELETE RESTRICT,
	previous_version BIGINT REFERENCES files(id) ON UPDATE CASCADE ON DELETE SET NULL
);
