...
bool SqliteDriver::create_db(sqlite3 * connection) {
	const char * queries[] = {
		...
		"CREATE TABLE files (id INTEGER PRIMARY KEY AUTOINCREMENT, hash_algo TEXT NOT NULL CHECK (hash_algo IN ('md5', 'sha1', 'sha256', 'sha512')), hash BLOB NOT NULL, data BLOB NOT NULL, host INTEGER NULL REFERENCES host(id) ON UPDATE CASCADE ON DELETE RESTRICT)",
		...
		"CREATE TABLE backups (id INTEGER PRIMARY KEY AUTOINCREMENT, start_time INTEGER NOT NULL, end_time INTEGER, size INTEGER CHECK (size >= 0), increment_size INTEGER CHECK (increment_size >= 0), parent_backup INTEGER REFERENCES backups(id) ON UPDATE CASCADE ON DELETE SET NULL)",
